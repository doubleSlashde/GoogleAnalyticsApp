...
public class GABrowserController {
   ...
   public void start() {  
      ...
      addBtn.setOnAction( (event) -> { addOffer(); }  );
      removeBtn.setOnAction( (event) -> { removeOffer(); }  );
   ...
   private GaData getGAData(String id, String path) {
      ...
      try {
         ...
         Get get = analytics.data().ga()
                        .get("ga:" + id,  "2015-07-01", "today", "ga:users,ga:avgTimeOnPage")
                        .setDimensions("ga:year,ga:month");
         
         if( path != null && path.length() > 0) {
            get = get.setFilters("ga:pagePath=~("+path+")");
         }
         
      // get = get.setFilters("ga:medium==organic")
   ...
   private void addOffer() {
      try {
         ...
         if( controller.isSave() ) {
            OfferSite offer = new OfferSite( controller.getOffername(), 
                                             controller.getViewid(), 
                                             controller.getPath());
 ...
 /*     GaData data = analytics.data().ga().get("ga:" + profileId, // Table Id. ga: + profile id.
                                                ...
                                                .execute();
     */ 
      GaData data = analytics.data().ga().get("ga:" + profileId,  "2015-12-01", "2016-01-10", "ga:users") 
                    .execute();
